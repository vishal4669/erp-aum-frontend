...
function EditQuotation(props) {
  ...
  const edit_quotation_id =url.substring(url.lastIndexOf('/') + 1)
  ...
  const [dataFor2, setForData2] = useState([]);

  // set test data dropdown
  const [data3, setData3] = useState([]);
  const [dataFor3, setForData3] = useState([]);

  // setting quotation fields array
  const [quotation, setQuotation] = useState({ quotation_no: '', type: 'Sample',customer_id:'',subject:'',
  quotation_date:'',valid_until:'',status:'Open',kind_attention:'',turn_around_time:'',remarks:'',currency_type:'1',grand_total:'1',
...
axios.all([axiosrequest1,axiosrequest3,axiosrequest4,axiosrequest5]).then(axios.spread(function(res1, res3,res4,res5) {
  setQuotation(res1.data.data[0]);
  setInputList(res1.data.data[0].quotation_product_info)
  if(res1.data.data[0].customer_dropdown){
  ...
  if(Array.isArray(res1.data.data[0].quotation_product_info) && res1.data.data[0].quotation_product_info.length){
    setInputList(res1.data.data[0].quotation_product_info);
    var product_data = res1.data.data[0].quotation_product_info.map(d =>
      d.product_dropdown.map(product =>product)
    )
      setData2(product_data)

      var test_data = res1.data.data[0].quotation_product_info.map(d =>
  ...
  if(res4){
      setForData2(res4.data.data)
  }

  if(res5){
      setForData3(res5.data.data)
  }
...
const EditQuotation = (e)=>{
        ...
        console.log(data)
         axios.post( `${process.env.REACT_APP_BASE_APIURL}updateQuotation/`+quotation_id, data, {headers} )
                .then(response => {
                    if(response.data.success == true){
                      ...
                      {setLoading(false)};
                        ...
                        props.history.push('/edit-quotation/'+edit_quotation_id);
...
const handleAddClick = (e) => {
  ...
  setInputList([...inputList,{sample_name: get_last_data.sample_name ? get_last_data.sample_name : '',
  product_name:get_last_data.product_name ? get_last_data.sample_name+" - "+get_last_data.product_name : '',
    test_required: get_last_data.test_required ? get_last_data.test_required : '',
    procedure_name: get_last_data.test_required ? get_last_data.test_required+" - "+get_last_data.procedure_name : '',
...
const handleInputChange = (e, index) => {
  if(e.target.name == "product_name"){
    ...
    setInputList(list);
  } else if(e.target.name == "procedure_name") {
    const list = [...inputList];
    list[index]["procedure_name"] = e.target.value;
      ...
      <HorizontalLayout/>
                ...
                <div className="page-content">
                    <div className="container-fluid">
                     <Form onSubmit={EditQuotation} method="POST" id="AddQuotation" name="QuotationData">
                        <div className="row">
                                            ...
                                            <div className="row">
                                                ...
                                                <div className="col-md-3">
                                                    ...
                                                    { loading1 ? <LoadingSpinner /> :
                                                       <select className="form-select" onChange={ onChange } name="type" value={quotation.type}>
                                                ...
                                                <div className="col-md-3">
                                                    <label className="required-field">Customer</label>
                                                     ...
                                                     {loading1 ? <center><LoadingSpinner /></center> : <Select value={data.find(obj => obj.value === quotation.customer_id)} onChange={e => onChangeCustomer(e) } options={data} name="customer_id"
                                                       placeholder="Select Customer" isClearable />}
                                                </div>
                                                <div className="col-md-3">
                                                    <label className="required-field">Subject</label>
                                                    <input value={quotation.subject} onChange={ onChange } className="form-control" type="text"  name="subject" placeholder="Enter Subject" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="form-group">
                                            <div className="row">
                                                ...
                                                <div className="col-md-3">
                                                    ...
                                                    <input value={quotation.quotation_date} onChange={ onChange } className="form-control" type="datetime-local" name="quotation_date" />
                                                </div>

                                                 <div className="col-md-3">
                                                    <label className="required-field">Valid Until</label>
                                                    <input value={quotation.valid_until} onChange={ onChange } className="form-control" type="datetime-local"  name="valid_until" />
                                                </div>

                                                 <div className="col-md-2">
                                                    <label>Status</label>
                                                    { loading1 ? <LoadingSpinner /> :
                                                       <select onChange={ onChange } className="form-select" name="status" value={quotation.status}>
                                                ...
                                                <div className="col-md-2">
                                                    ...
                                                    <input value={quotation.kind_attention} onChange={ onChange } className="form-control" type="text"  name="kind_attention" placeholder="Kind Attention Detail" />
                                                </div>

                                                 <div className="col-md-2">
                                                    <label className="required-field">Turn Around Time</label>
                                                    <input value={quotation.turn_around_time} onChange={ onChange } className="form-control" type="text"  name="turn_around_time" placeholder="Enter Turn Around Time" />
                                                </div>
                                        ...
                                        <div className="form-group">
                                            <div className="row">
                                                ...
                                                <div className="col-md-6">
                                                    <label>Remarks</label>
                                                    <textarea value={quotation.remarks} onChange={ onChange } name="remarks" className="form-control" placeholder="Enter Remarks"></textarea>
                                                </div>
                                                ...
                                                <div className="col-md-2">
                                                 ...
                                                 <div className="col-md-2">
                                                    ...
                                                    { loading1 ? <LoadingSpinner /> :
                                                       <select onChange={ onChange } className="form-select" name="grand_total" value={quotation.grand_total}>
                                                ...
                                                <div className="col-md-2">
                                                    ...
                                                    <input onChange={ onChange } value={quotation.payment_terms} className="form-control" type="text"  name="payment_terms" placeholder="Enter Payment Terms" />
                                                </div>
                                                            ...
                                                            <tr>
                                                                <td>
                                                                 <input class="form-control" value={x.product_name} name="product_name" list="sample_name" id="product_list"  placeholder="Type to search..." onChange={e => handleInputChange(e, i)}/>
                                                                    ...
                                                                    { data2[i] ?
                                                                    (data2[i].map((option,key) =>
                                                                      <option value={option.id+" - "+option.product_name}></option>
                                                                    )) :  (dataFor2.map((option,key) =>
                                                                       <option value={option.id+" - "+option.product_name}></option>
                                                                     ))}
                                                                    </datalist>
                                                                </td>
                                                                <td>
                                                                <input class="form-control" value={x.procedure_name} name="procedure_name" list="test_required" id="test_list"  placeholder="Type to search..." onChange={e => handleInputChange(e, i)}/>
                                                                   <datalist id="test_required">
                                                                   { data3[i] ?
                                                                   (data3[i].map((option,key) =>
                                                                     <option value={option.id+" - "+option.procedure_name}></option>
                                                                   )) :  (dataFor3.map((option,key) =>
                                                                      <option value={option.id+" - "+option.procedure_name}></option>
                                                                    ))}
                                                                   </datalist>
